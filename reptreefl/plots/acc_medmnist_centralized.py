...
for method, run_name in zip(methods, run_names):
    for fold in range(NUMBER_FOLDS):
        ...
        if 'centralized' in run_name:
            for client in range(NUMBER_CLIENTS):
                mae_all[str(client) + method].append(mae_current_fold[0])
        else:
            for client in range(NUMBER_CLIENTS):
...
plt.savefig(f'{SAVING_FOLDER_PATH}/{GLOBAL_RUN_NAME}/average_runs_acc_centralized.png')
